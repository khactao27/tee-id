...
export default class AuthService {
    ...
    logIn = function(tracker, type, cb) {
        if(type == AccountTypes.TEE_ID) {
            IDService.login((error, result) => {
                if(error) { 
                    if(tracker) {
    ...
    getCurrentLoggedInUser = function(tracker, type, cb) {
        if(type == AccountTypes.TEE_ID) {
            IDService.getUserInfo((err, accessToken, userInfo) => {
                if(err) { 
                    if(tracker) {
                        tracker.setUser(GA_UNKNOWN_USER);
                    }
    ...
    saveFbUser = function(userId, userInfoInString, cb) {
        return this._userRepository.getByTypeAndId(AccountTypes.TEE_ID, userId,  (error, data) => {
            if(error) { cb(error); }
            else {
                if(data && data.id != undefined) {
                    // update
                    this._userRepository.update(AccountTypes.TEE_ID, userId, new User(data.id, data.type, data.userId, userInfoInString, true), cb);
                } else {
                    // create
                    this._userRepository.create(new User(StringUtil.guid(), AccountTypes.TEE_ID, userId, userInfoInString, true), cb);
                }
            }
        });
    }
